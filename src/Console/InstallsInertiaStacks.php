...
trait InstallsInertiaStacks
{
    ...
    /**
     * Install the Inertia React Typescript Breeze stack.
     *
     * @return int|null
     */
    protected function installInertiaReactTypescriptStack()
    {
        ...
        copy(__DIR__ . '/../../stubs/default/postcss.config.js', base_path('postcss.config.js'));
        copy(__DIR__ . '/../../stubs/inertia-common/tailwind.config.js', base_path('tailwind.config.js'));
        copy(__DIR__ . '/../../stubs/inertia-common/tsconfig.json', base_path('tsconfig.json'));
        copy(__DIR__ . '/../../stubs/inertia-common/tsconfig.node.json', base_path('tsconfig.node.json'));
        ...
        $this->replaceInFile('.vue', '.{js,ts,jsx,tsx}', base_path('tailwind.config.js'));

        if ($this->option('ssr')) {
            $this->installInertiaReactTypescriptSsrStack();
        }
     ...
     * @return void
     */
    protected function installInertiaReactTypescriptSsrStack()
    {
        copy(__DIR__ . '/../../stubs/inertia-react-ts/resources/js/ssr.tsx', resource_path('js/ssr.tsx'));
        $this->replaceInFile("input: 'resources/js/app.tsx',", "input: 'resources/js/app.tsx'," . PHP_EOL . "            ssr: 'resources/js/ssr.tsx',", base_path('vite.config.ts'));
        $this->replaceInFile('/node_modules', '/bootstrap/ssr' . PHP_EOL . '/node_modules', base_path('.gitignore'));

        $this->updateNodeScripts([
            "build" => "vite build && vite build --ssr",
            "preview" => "npm run build && node bootstrap/ssr/ssr.mjs",
            "test-ssr" => "npm run build && node bootstrap/ssr/ssr.mjs"
        ]);
    }
}
